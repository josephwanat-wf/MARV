...
function handleSubmitClicked() {
	if ($('#newDate').val() != "") {
		if ($('#fixVersion').val() != ""){
			...
			var UXT = $('#UXT').is(':checked');
			var CRN = $('#CRN').is(':checked');
			var EPT = $('#EPT').is(':checked');
			var FF = $('#FF').is(':checked');
			var HVR = $('#HVR').is(':checked');
			...
			chrome.extension.getBackgroundPage().mgr.updateVariables(month, day, year, fixVersion, AD, ADMIN, AR, SKY, CERT, DI, DR, DS, FAP, FT, FUI, IAPI, PRES, SIXTEEN, SSC, SHEET, TRA, XBRLSEC, XL, XS, XW, HC, LOG, BOOKS, FUN, INF, RR, FETT, UXT, CRN, EPT, FF, HVR, description);
		}
...
function handleSubmit2Clicked() {
	if ($('#fixVersion2').val() != "") {
		...
		var FF = $('#FF').is(':checked');
		var HVR = $('#HVR').is(':checked');
		chrome.extension.getBackgroundPage().mgr.updateVariables2(fixVersion2, AD, ADMIN, AR, SKY, CERT, DI, DR, DS, FAP, FT, FUI, IAPI, PRES, SIXTEEN, SSC, SHEET, TRA, XBRLSEC, XL, XS, XW, HC, LOG, BOOKS, FUN, INF, RR, FETT, UXT, CRN, EPT, FF, HVR);	
	}
	else alert("Please choose a version");
}
function handleSubmit3Clicked() {
	if ($('#date3').val() != "") {
		if ($('#fixVersion3').val() != ""){
			...
			chrome.extension.getBackgroundPage().mgr.updateVariables3(fixVersion3, year, month, day, AD, ADMIN, AR, SKY, CERT, DI, DR, DS, FAP, FT, FUI, IAPI, PRES, SIXTEEN, SSC, SHEET, TRA, XBRLSEC, XL, XS, XW, HC, LOG, BOOKS, FUN, INF, RR, FETT, UXT, CRN, EPT, FF, HVR);
		}
		else alert("Please choose a version");
	}
	else alert("Please enter a date");
...
function handleHideProjClicked(){
	...
	$('#tUXT').attr('class', 'hidden');
	$('#tCRN').attr('class', 'hidden');
	$('#tEPT').attr('class', 'hidden');
	$('#tFF').attr('class', 'hidden');
	$('#tHVR').attr('class', 'hidden');
...
function handleShowProjClicked(){
	...
	$('#tUXT').attr('class', 'shown');
	$('#tCRN').attr('class', 'shown');
	$('#tEPT').attr('class', 'shown');
	$('#tFF').attr('class', 'shown');
	$('#tHVR').attr('class', 'shown');
